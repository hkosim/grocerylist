...
public class JdbcItemRepository{
    ...
    public void create(Item Item) {
        var updated = jdbcClient.sql("INSERT INTO item(id, name, description, quantity, category, status, supermarket_name, purchased_on) values(?,?,?,?,?,?,?,?)")
                .params (
                        List.of(
                            Item.id(),
                            Item.name(),
    ...
    public void update(Item Item, Integer id) {
        Optional<Item> itemInDb = findById(id);

        if (itemInDb.isPresent()) {

            var updated = jdbcClient.sql("update item set name=?, description=?, quantity=?, category=?, status=?, supermarket_name=?, purchased_on=?, version=?  where id = ?")
                    .params (
                            List.of(
                                    Item.name(),
                                    Item.description(),
